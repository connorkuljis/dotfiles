...
--- Configuration options for radio_chatter
---@class RadioChatterConfig
---@field enabled boolean Whether to enable the plugin (default: true)
---@field backend string TTS backend to use (default: "auto")
---@field phrases? table<string, string>|nil Default phrases per event (default: various)
---@field espeak? table|nil Configuration for espeak backend (default: { rate = 200, pitch = 40, voice = nil })
---@field spdsay? table|nil Configuration for spd-say backend (default: { voice = nil, rate = "+20" })
---@field pico? table|nil Configuration for pico2wave backend (default: { lang = "en-GB", player = "paplay" })
---@field piper? table|nil Configuration for piper backend (default: various)

---@type RadioChatterConfig
M.config = {
	...
	phrases = {
		...
		VimLeavePre = "Knocking off.",
	},
	-- tuning
	espeak = { rate = 200, pitch = 40, voice = nil },
	spdsay = { voice = nil, rate = "+20" }, -- e.g., { voice = "male3", rate = "-20" }
...
local function build_cmd(text)
	...
	if b == "spd-say" then
		local c = { "spd-say" }
		if M.config.spdsay.voice then
			vim.list_extend(c, { "-o", M.config.spdsay.voice })
		end
		if M.config.spdsay.rate then
			vim.list_extend(c, { "-r", M.config.spdsay.rate })
		end
		table.insert(c, text)
		return c
	end
	...
	if b == "espeak-ng" then
		local c = { "espeak-ng" }
		if M.config.espeak.voice then
			vim.list_extend(c, { "-v", M.config.espeak.voice })
		end
		if M.config.espeak.rate then
			vim.list_extend(c, { "-s", tostring(M.config.espeak.rate) })
		end
		if M.config.espeak.pitch then
			vim.list_extend(c, { "-p", tostring(M.config.espeak.pitch) })
		end
		table.insert(c, text)
		return c
	end
	...
	if b == "pico2wave" then
		...
		local c = {
			...
			"-lc",
			string.format(
				"pico2wave -l %s -w %q %q && %s %q && rm -f %q",
				M.config.pico.lang,
				tmp,
				text,
				M.config.pico.player,
				tmp,
				tmp
			),
		}
...
---@param opts? RadioChatterConfig|nil Optional configuration options
